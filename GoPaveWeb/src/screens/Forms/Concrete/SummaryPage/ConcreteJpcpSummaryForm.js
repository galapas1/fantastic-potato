...
const concreteJpcpSummaryTooltipMessage = 'Dowels are not typically used in pavements <br />  less than 7 inches thick. Conventional round dowels are <br />  more difficult to use in thin pavements. Alternative dowel technologies<br />   and geometries  (such as plate dowels) may be considered if <br />  faulting is still a significant concern';
const concreteJpcpSummaryTooltipID = 'concreteJpcpSummaryRadioButton';

class ConcreteJpcpSummaryForm extends Component {
...
2. 	A general rule-of-thumb requires that the transverse joint spacing should not exceed 150% of the longitudinal joint spacing.  
    ...
    render() {
        ...
        let tooltipAndStylesForDowelslessThanSevenInches = {
            ...(parseFloat(this.props[JPCP_DOWELED_DESIGN_THICKNESS]) < 7 && { 'showTooltip': true, 'tooltipId': concreteJpcpSummaryTooltipID, 'toolTipMessage': concreteJpcpSummaryTooltipMessage, customSpanStyle, customInputStyle })
        };
        return (
            <div className='summary-page' style={{ zIndex: 10, paddingTop: 10, display: "flex", flex: "1 1 0%", flexDirection: "column", justifyContent: "space-between", overflowY: 'auto' }}>
                    ...
                    <div style={{ width: "30%", display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 20 }}>
                        ...
                        <div className='form-group-div' style={{ width: '100%' }}>
                            ...
                            <div ref='RecommendedDesignThickness' style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 'inherit' }}>
                                    <div style={{ paddingBottom: 20, fontSize: 12 }}>Doweled</div>
                                    <ReactTooltip id={concreteJpcpSummaryTooltipID} multiline={true} place='right' type='warning' effect='solid' />
                ...
                <Popup concreteType='JPCP' jpcpSummaryThicknessOption={this.props.jpcpSummaryFormThicknessToggle} popup={CONCRETE_SUMMARY_FORM_GRAPH_POPUP} />
            </div>
        );
    }
...
function mapStateToProps(state, ownProps, context) {
    ...
    const jpcpDoweledKValueGraphData = state.currentProject.concreteFormValues.jpcpDoweledKValueGraphData;
    const jpcpUndoweledKValueGraphData = state.currentProject.concreteFormValues.jpcpUndoweledKValueGraphData;
    const jpcpDoweledFlexuralStrengthGraphData = state.currentProject.concreteFormValues.jpcpDoweledFlexuralStrengthGraphData;
    const jpcpUndoweledFlexuralStrengthGraphData = state.currentProject.concreteFormValues.jpcpUndoweledFlexuralStrengthGraphData;
    const jpcpDoweledReliabilityGraphData = state.currentProject.concreteFormValues.jpcpDoweledReliabilityGraphData;
    const jpcpUndoweledReliabilityGraphData = state.currentProject.concreteFormValues.jpcpUndoweledReliabilityGraphData;
    const jpcpDoweledDesignLifeGraphData = state.currentProject.concreteFormValues.jpcpDoweledDesignLifeGraphData;
    const jpcpUndoweledDesignLifeGraphData = state.currentProject.concreteFormValues.jpcpUndoweledDesignLifeGraphData;
    const jpcpDoweledSlabsCrackedGraphData = state.currentProject.concreteFormValues.jpcpDoweledSlabsCrackedGraphData;
    const jpcpUndoweledSlabsCrackedGraphData = state.currentProject.concreteFormValues.jpcpUndoweledSlabsCrackedGraphData;

    const doweledCrackingValues = state.currentProject.concreteFormValues.jpcpDoweledCrackingValues;
    const undoweledCrackingValues = state.currentProject.concreteFormValues.jpcpUndoweledCrackingValues;
    const doweledFaultingValues = state.currentProject.concreteFormValues.jpcpDoweledFaultingValues;
    const undoweledFaultingValues = state.currentProject.concreteFormValues.jpcpUndoweledFaultingValues;

    const doweledFatigueUsedValue = state.currentProject.concreteFormValues.jpcpDoweledFatigueUsedValue;
    const undoweledFatigueUsedValue = state.currentProject.concreteFormValues.jpcpUndoweledFatigueUsedValue;
    const doweledErosionUsedValue = state.currentProject.concreteFormValues.jpcpDoweledErosionUsedValue;
    const undoweledErosionUsedValue = state.currentProject.concreteFormValues.jpcpUndoweledErosionUsedValue;
        ...
        generatePdf: (summaryContext) => {
            ...
            let imgRefs = [{
                ...
                fields: [
                    ...
                    { type: 'text', text: 'JOINTED-PLAIN CONCRETE PAVEMENT (JPCP)', value: '', x: 200, y: 35, xPad: 70, lg: true },
...
ConcreteJpcpSummaryForm = connect(
    mapStateToProps, mapDispatchToProps,
)(ConcreteJpcpSummaryForm)

export default ConcreteJpcpSummaryForm;
