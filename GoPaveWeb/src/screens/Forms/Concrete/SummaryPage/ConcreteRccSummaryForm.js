...
class ConcreteRccSummaryForm extends Component {
    ...
    render() {
        ...
        return (
            <div className='summary-page' style={{ zIndex: 10, paddingTop: 10, display: "flex", flex: "1 1 0%", flexDirection: "column", justifyContent: "space-between" }}>
                ...
                <Popup concreteType='RCC' popup={CONCRETE_SUMMARY_FORM_GRAPH_POPUP} />
            </div>
        );
    }
...
ConcreteRccSummaryForm = reduxForm({
    form: CONCRETE_RCC_SUMMARY_FORM,
    destroyOnUnmount: false,
    initialValues: {
        [SUMMARY_FORM_TOGGLE]: SUMMARY_FORM_SENSITIVITY
    }
})(ConcreteRccSummaryForm);

function mapStateToProps(state, ownProps, context) {
    const selector = formValueSelector(CONCRETE_RCC_SUMMARY_FORM);
    const summaryFormToggle = selector(state, SUMMARY_FORM_TOGGLE);
    const rccKValueGraphData = state.currentProject.concreteFormValues.rccKValueGraphData;
    const rccFlexuralStrengthGraphData = state.currentProject.concreteFormValues.rccFlexuralStrengthGraphData;
    const rccReliabilityGraphData = state.currentProject.concreteFormValues.rccReliabilityGraphData;
    const rccDesignLifeGraphData = state.currentProject.concreteFormValues.rccDesignLifeGraphData;
    const rccSlabsCrackedGraphData = state.currentProject.concreteFormValues.rccSlabsCrackedGraphData;
    ...
    const rccCrackingValues = state.currentProject.concreteFormValues.rccCrackingValues;
    const rccFaultingValues = state.currentProject.concreteFormValues.rccFaultingValues;
    const rccErosionUsedValue = state.currentProject.concreteFormValues.rccErosionUsedValue;
    const rccFatigueUsedValue = state.currentProject.concreteFormValues.rccFatigueUsedValue;
        ...
        generatePdf: (summaryContext) => {
            ...
            let imgRefs = [{
                ...
                fields: [
                    ...
                    { type: 'text', text: 'ROLLER-COMPACTED CONCRETE PAVEMENT (RCC)', value: '', x: 200, y: 35, xPad: 70, lg: true },
                ...
                },
                {
                    ...
                    fields: [
                        { type: 'text', text: '',
                            value: 'Undoweled' + '            ', x: 5, y: 5, xPad: 125, sm: true },
...
ConcreteRccSummaryForm = connect(
    mapStateToProps, mapDispatchToProps,
)(ConcreteRccSummaryForm)

export default ConcreteRccSummaryForm;
