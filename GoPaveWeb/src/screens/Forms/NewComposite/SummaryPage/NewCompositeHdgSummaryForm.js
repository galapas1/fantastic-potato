...
class NewCompositeHdgSummaryForm extends Component {
...
NewCompositeHdgSummaryForm = reduxForm({
    form: NEW_COMPOSITE_HDG_SUMMARY_FORM,
    destroyOnUnmount: false,
})(NewCompositeHdgSummaryForm);
...
function mapStateToProps(state, ownProps, context) {
    ...
    const graphData = state.currentProject.newCompositeFormValues.asphaltSummaryHdgGraphValues;
    const graphReferenceLines = state.currentProject.newCompositeFormValues.asphaltSummaryHdgGraphDamageReferenceLinesValues;
    const subgradeLayerSelector = formValueSelector(NEW_COMPOSITE_HMA_SUBGRADE_FORM)
    const thicknesstorigidfoundation = subgradeLayerSelector(state, THICKNESS_TO_RIGID_FOUNDATION);


    const surfaceLayerSelector = formValueSelector(NEW_COMPOSITE_HMA_SURFACE_LAYER_FORM)
    ...
    const designLayerValues = state.currentProject.newCompositeFormValues.asphaltSummaryHdgGraphDesignlayerValues
    ...
    const summarySelector = formValueSelector(NEW_COMPOSITE_HDG_SUMMARY_FORM);
    const minimumRequiredThickness = summarySelector(state, MINIMUM_REQUIRED_THICKNESS );

    const globalForm = getFormValues(NEW_COMPOSITE_HDG_SUMMARY_FORM)(state);
    ...
    const concreteSubgradeForm = getFormValues(NEW_COMPOSITE_HMA_SUBGRADE_FORM)(state);
    const concreteSubbaseForm = getFormValues(NEW_COMPOSITE_HMA_STRUCTURE_FORM)(state);
    const concreteConcreteForm = getFormValues(NEW_COMPOSITE_HMA_SURFACE_LAYER_FORM)(state);
        ...
        generatePdf: (summaryContext) => {
            ...
            let imgRefs = [{
                ...
                fields: [
                    ...
                    { type: 'text', text: 'ANALYSIS REPORT FOR COMPOSITE HMA/WMA DENSE GRADED PAVEMENT', value: '', x: 200, y: 20, xPad: 70, lg: true },
            ...
            },
            {
                ...
                fields: [
                    { type: 'text', text: 'HMA/WMA SURFACE LAYER', value: '', x: 5, y: 15, xPad: 125 },
...
NewCompositeHdgSummaryForm = connect(
    mapStateToProps, mapDispatchToProps,
)(NewCompositeHdgSummaryForm)


export default NewCompositeHdgSummaryForm;
