...
const overlayUnbondedAsphaltJpcpSummaryTooltipMessage = 'Dowels are not typically used in pavements <br />  less than 7 inches thick. Conventional round dowels are <br />  more difficult to use in thin pavements. Alternative dowel technologies<br />   and geometries  (such as plate dowels) may be considered if <br />  faulting is still a significant concern';
const overlayUnbondedAsphaltJpcpSummaryTooltipID = 'concreteJpcpSummaryRadioButton';

class OverlayUnbondedAsphaltSummaryForm extends Component {
        ...
        if (this.props[SUMMARY_FORM_TOGGLE] === SUMMARY_FORM_SENSITIVITY) {
            ...
            return (
                <div style={{ height: '100%' }}>
                    <div style={{ display: 'flex', width: '82%', justifyContent: 'space-between', fontSize: 12, fontWeight: 'bold', marginBottom: 10, marginTop: 10 }}>
                        ...
                        <div style={{ fontSize: 11 }}>{`TOTAL FATIGUE USED %: ${dowelOption === SUMMARY_FORM_DOWELED ? this.props.unbondedAsphaltDoweledFatigueUsedValue : this.props.unbondedAsphaltUndoweledFatigueUsedValue}`}</div>
                    </div>
            ...
            return (
                <div style={{ height: '100%' }}>
                    <div style={{ display: 'flex', width: '82%', justifyContent: 'space-between', fontSize: 12, fontWeight: 'bold', marginBottom: 10, marginTop: 10 }}>
                        <div style={{ fontSize: 11 }}>{`TRAFFIC SUMMARY DETAILS: ${this.props.trafficSummaryDropdownValue}`}</div>
                        <div style={{ fontSize: 11 }}>{`TOTAL EROSION USED %: ${dowelOption === SUMMARY_FORM_DOWELED ? this.props.unbondedAsphaltDoweledErosionUsedValue : this.props.unbondedAsphaltUndoweledErosionUsedValue}`}</div>
                    </div>
                    <div style={{ width: '85%', height: '75%', overflowY: 'auto', minWidth: 400 }}>
                        <table style={{ width: '97%' }}>
                            <tbody style={{ fontSize: 11, textAlign: 'center' }}>
    ...
    render() {
        ...
        let tooltipAndStylesForDowelslessThanSevenInches = {
            ...(parseFloat(this.props[JPCP_DOWELED_DESIGN_THICKNESS]) < 7 && { 'showTooltip': true, 'tooltipId': overlayUnbondedAsphaltJpcpSummaryTooltipID, 'toolTipMessage': overlayUnbondedAsphaltJpcpSummaryTooltipMessage, customSpanStyle, customInputStyle })
        };
        return (
            <div className='summary-page' style={{ zIndex: 10, paddingTop: 10, display: 'flex', flex: '1 1 0%', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div style={{ paddingLeft: 30, paddingTop: 10 }}>
                    <div style={{ paddingLeft: 30, display: 'flex', width: '500px', borderBottomLeftRadius: '10px', borderTopLeftRadius: '10px', borderBottomRightRadius: '10px', borderTopRightRadius: '10px', backgroundColor: 'whitesmoke', fontWeight: 'bold' }}>
                        ...
                        <div style={{ paddingRight: 10 }}><i className="fa fa-chevron-circle-right"></i></div>
                        <div onClick={browserHistory.goBack} data-tip={radioKButtonTooltipMessage} data-for={radioKButtonTooltipID} style={{ paddingLeft: 5, color: "#3299CC", textDecoration: 'underline', cursor: 'pointer', fontSize: 16 }}>Unbonded Asphalt</div>
                    </div></div>
                <div style={{ display: 'flex', width: 'calc(100% - 100px)', height: '100%', margin: '0 auto', flex: 1 }}>
                    <div style={{ width: '30%', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 20 }}>
                        <div className='form-group-div' style={{ width: '100%', marginBottom: 20 }}>
                            <label style={{ paddingBottom: 20, fontSize: 15 }}>Calculated Minimum Required Thickness</label>
                        ...
                        <div className='form-group-div' style={{ width: '100%', marginBottom: 20 }}>
                            ...
                            <div ref='RecommendedDesignThickness' style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 'inherit' }}>
                                    <div style={{ paddingBottom: 20, fontSize: 12 }}>Doweled</div>
                                    <ReactTooltip id={overlayUnbondedAsphaltJpcpSummaryTooltipID} multiline={true} place='right' type='warning' effect='solid' />
                ...
                <Popup type={UNBONDED_ASPHALT} jpcpSummaryThicknessOption={this.props.jpcpSummaryFormThicknessToggle} popup={OVERLAY_SUMMARY_FORM_GRAPH_POPUP} />
            </div>
        );
    }
...
OverlayUnbondedAsphaltSummaryForm = reduxForm({
    form: OVERLAY_UNBONDED_ASPHALT_SUMMARY_FORM,
    destroyOnUnmount: false,
    initialValues: {
        [SUMMARY_FORM_TOGGLE]: SUMMARY_FORM_SENSITIVITY,
        [JPCP_SUMMARY_FORM_THICKNESS_TOGGLE]: SUMMARY_FORM_DOWELED
    }
})(OverlayUnbondedAsphaltSummaryForm);

function mapStateToProps(state, ownProps, context) {
    const selector = formValueSelector(OVERLAY_UNBONDED_ASPHALT_SUMMARY_FORM);
    ...
    const unbondedAsphaltDoweledKValueGraphData = state.currentProject.overlayFormValues.unbondedAsphaltDoweledKValueGraphData;
    const unbondedAsphaltUndoweledKValueGraphData = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledKValueGraphData;
    const unbondedAsphaltDoweledFlexuralStrengthGraphData = state.currentProject.overlayFormValues.unbondedAsphaltDoweledFlexuralStrengthGraphData;
    const unbondedAsphaltUndoweledFlexuralStrengthGraphData = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledFlexuralStrengthGraphData;
    const unbondedAsphaltDoweledReliabilityGraphData = state.currentProject.overlayFormValues.unbondedAsphaltDoweledReliabilityGraphData;
    const unbondedAsphaltUndoweledReliabilityGraphData = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledReliabilityGraphData;
    const unbondedAsphaltDoweledDesignLifeGraphData = state.currentProject.overlayFormValues.unbondedAsphaltDoweledDesignLifeGraphData;
    const unbondedAsphaltUndoweledDesignLifeGraphData = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledDesignLifeGraphData;
    const unbondedAsphaltDoweledSlabsCrackedGraphData = state.currentProject.overlayFormValues.unbondedAsphaltDoweledSlabsCrackedGraphData;
    const unbondedAsphaltUndoweledSlabsCrackedGraphData = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledSlabsCrackedGraphData;

    const unbondedAsphaltDoweledCrackingValues = state.currentProject.overlayFormValues.unbondedAsphaltDoweledCrackingValues;
    const unbondedAsphaltUndoweledCrackingValues = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledCrackingValues;
    const unbondedAsphaltDoweledFaultingValues = state.currentProject.overlayFormValues.unbondedAsphaltDoweledFaultingValues;
    const unbondedAsphaltUndoweledFaultingValues = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledFaultingValues;

    const unbondedAsphaltDoweledFatigueUsedValue = state.currentProject.overlayFormValues.unbondedAsphaltDoweledFatigueUsedValue;
    const unbondedAsphaltUndoweledFatigueUsedValue = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledFatigueUsedValue;
    const unbondedAsphaltDoweledErosionUsedValue = state.currentProject.overlayFormValues.unbondedAsphaltDoweledErosionUsedValue;
    const unbondedAsphaltUndoweledErosionUsedValue = state.currentProject.overlayFormValues.unbondedAsphaltUndoweledErosionUsedValue;
    ...
    const concreteSubgradeForm = getFormValues(OVERLAY_UNBONDED_ASPHALT_SUBGRADE_FORM)(state);
    const concreteSubbaseForm = getFormValues(OVERLAY_UNBONDED_ASPHALT_SUBBASE_FORM)(state);
    const concreteConcreteForm = getFormValues(OVERLAY_UNBONDED_ASPHALT_CONCRETE_FORM)(state);
    ...
    return {
        ...
        unbondedAsphaltDoweledKValueGraphData,
        unbondedAsphaltUndoweledKValueGraphData,
        unbondedAsphaltDoweledFlexuralStrengthGraphData,
        unbondedAsphaltUndoweledFlexuralStrengthGraphData,
        unbondedAsphaltDoweledReliabilityGraphData,
        unbondedAsphaltUndoweledReliabilityGraphData,
        unbondedAsphaltDoweledDesignLifeGraphData,
        unbondedAsphaltUndoweledDesignLifeGraphData,
        jpcpSummaryFormThicknessToggle,
        unbondedAsphaltDoweledCrackingValues,
        unbondedAsphaltUndoweledCrackingValues,
        unbondedAsphaltDoweledFaultingValues,
        unbondedAsphaltUndoweledFaultingValues,
        trafficSummaryDropdownValue,
        unbondedAsphaltDoweledFatigueUsedValue,
        unbondedAsphaltUndoweledFatigueUsedValue,
        unbondedAsphaltDoweledErosionUsedValue,
        unbondedAsphaltUndoweledErosionUsedValue,
        unbondedAsphaltDoweledSlabsCrackedGraphData,
        unbondedAsphaltUndoweledSlabsCrackedGraphData,
        jpcpDoweledDesignThickness,
        unitType,
    };
}
        ...
        generatePdf: (summaryContext) => {
            ...
            let imgRefs = [{
                ...
                fields: [
                    ...
                    { type: 'text', text: 'UNBONDED CONCRETE ON ASPHALT OVERLAY', value: '', x: 200, y: 35, xPad: 70, lg: true },
...
OverlayUnbondedAsphaltSummaryForm = connect(
    mapStateToProps, mapDispatchToProps,
)(OverlayUnbondedAsphaltSummaryForm)

export default OverlayUnbondedAsphaltSummaryForm;
