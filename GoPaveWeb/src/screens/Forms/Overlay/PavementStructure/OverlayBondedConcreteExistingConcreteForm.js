...
import { Field, reduxForm, formValueSelector, getFormSyncErrors, change, untouch, initialize, destroy  } from 'redux-form';

import { calculateJoingCrackingAdjustmentFactor,
...
const HELP_BONDED_OVERLAY_FORM_FATIGUE_ADJUSTMENT_FACTOR_TOOLTIP_ID = 'helpBondedOverlayFormFatigueAdjustmentFactorTooltipId';
const HELP_BONDED_OVERLAY_FORM_DURABILITY_ADJUSTMENT_FACTOR_TOOLTIP_ID = 'helpBondedOverlayFormDurabilityAdjustmentFactorTooltipId';

class OverlayBondedExistingConcreteForm extends Component {
  ...
  calculateJointCrackingFactor() {
    ...
    extend(calculatorValues, {isBonded: true});

    if (this.props[EXISTING_THICKNESS] && this.props[JOINTS_AND_CRACKS_ADJUSTMENT_FACTOR] && this.props[DURABILITY_ADJUSTMENT_FACTOR] && this.props[FATIGUE_ADJUSTMENT_FACTOR]) {
      extend(calculatorValues, pick(this.props, EXISTING_THICKNESS, JOINTS_AND_CRACKS_ADJUSTMENT_FACTOR, DURABILITY_ADJUSTMENT_FACTOR, FATIGUE_ADJUSTMENT_FACTOR ));
      this.props.calculateJoingCrackingAdjustmentFactor(calculatorValues, true, true); 
    } else {
      this.props.calculateJoingCrackingAdjustmentFactor(calculatorValues, true, false); 
    }
  }

  calculateEffectiveThickness() {
    if (!this.props[EXISTING_THICKNESS] || !this.props[JOINTS_AND_CRACKS_ADJUSTMENT_FACTOR] || !this.props[FATIGUE_ADJUSTMENT_FACTOR] || !this.props[DURABILITY_ADJUSTMENT_FACTOR] ) {
     return;
    }
    const calculatorValues = pick(this.props, EXISTING_THICKNESS, JOINTS_AND_CRACKS_ADJUSTMENT_FACTOR, FATIGUE_ADJUSTMENT_FACTOR, DURABILITY_ADJUSTMENT_FACTOR);
    extend(calculatorValues, {isBonded: true});
    this.props.calculateEffectiveThickness(calculatorValues, true);
  }

  setJointAndCracksAdjustmentFactorNillCalculatorValues() {
    this.props.changeFieldValue(OVERLAY_BONDED_CONCRETE_EXISTING_CONCRETE_FORM, JOINTS_AND_CRACKS_ADJUSTMENT_FACTOR , ''); 
    this.props.changeFieldValue(OVERLAY_BONDED_CONCRETE_EXISTING_CONCRETE_FORM, EFFECTIVE_THICKNESS , '');
  }
        ...
        <div style={{height: "100%", width: "100%"}}>
          <div style={{display: "flex", alignItems: 'center', flexDirection: 'column', height: '100%', width: '100%', borderRight:  '1px solid'}}>
            <div style={{width: "100%"}}>
              ...
              <div className='form-group-div' style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%'}}>
                <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', width: '50%'}}>
                                <HelpScreenTooltip id={HELP_BONDED_OVERLAY_FORM_FATIGUE_ADJUSTMENT_FACTOR_TOOLTIP_ID} helpScreenType={FATIGUE_ADJUSTMENT_FACTOR_HELP_SCREEN} helpScreenTitle={SmallHelpScreenConfig[FATIGUE_ADJUSTMENT_FACTOR_HELP_SCREEN]['title']} showPopup={this.props.showPopup} popup={HELP_SCREEN_POPUP} firstParagraph={SmallHelpScreenConfig[FATIGUE_ADJUSTMENT_FACTOR_HELP_SCREEN]['firstParagraph'][this.props.unitType]} isShow={this.props.isHelp} />

                                <label style={{ fontSize: 9 }} >Fatigue Adjustment Factor</label>
                                <Field className='form-group-div-input' name={FATIGUE_ADJUSTMENT_FACTOR} calculateValues={this.calculateEffectiveThickness.bind(this)} setNillCalculatorValues={this.setEffectiveThicknessNillCalculatorValues.bind(this)} type="text" component={FieldInput} showHelpTooltip={this.props.isHelp} helptooltipId={HELP_BONDED_OVERLAY_FORM_FATIGUE_ADJUSTMENT_FACTOR_TOOLTIP_ID} />
                </div>
                <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', width: '50%'}}>
                                <HelpScreenTooltip id={HELP_BONDED_OVERLAY_FORM_DURABILITY_ADJUSTMENT_FACTOR_TOOLTIP_ID} helpScreenType={DURABILITY_ADJUSTMENT_FACTOR_HELP_SCREEN} helpScreenTitle={SmallHelpScreenConfig[DURABILITY_ADJUSTMENT_FACTOR_HELP_SCREEN]['title']} showPopup={this.props.showPopup} popup={HELP_SCREEN_POPUP} firstParagraph={SmallHelpScreenConfig[DURABILITY_ADJUSTMENT_FACTOR_HELP_SCREEN]['firstParagraph'][this.props.unitType]} isShow={this.props.isHelp} />

                                <label style={{ fontSize: 9 }} >Durability Adjustment Factor</label>
                  
                  <Field className='form-group-div-input' name={DURABILITY_ADJUSTMENT_FACTOR} calculateValues={this.calculateEffectiveThickness.bind(this)} setNillCalculatorValues={this.setEffectiveThicknessNillCalculatorValues.bind(this)} type="text" component={FieldInput} showHelpTooltip={this.props.isHelp} helptooltipId={HELP_BONDED_OVERLAY_FORM_DURABILITY_ADJUSTMENT_FACTOR_TOOLTIP_ID} />
                </div>
              </div>
              <div className='form-group-div' style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%'}}>
                 <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%'}}>
            ...
            <div style={{ width: "100%", textAlign: "center", paddingTop: 20, fontWeight: "bold", paddingBottom: 20}}>SUBGRADE</div>
             <div style={{height: "100%", width: "100%"}}>
               <MrsgForm kvalueOutputForm={OVERLAY_BONDED_CONCRETE_SUBBASE_FORM} mrsgOutputForm={OVERLAY_BONDED_CONCRETE_EXISTING_CONCRETE_FORM} module={OVERLAY_UA_MODULE} height='100%'  {...this.props} />
...
OverlayBondedExistingConcreteForm = reduxForm({
    form: OVERLAY_BONDED_CONCRETE_EXISTING_CONCRETE_FORM,
    validate,
    warn,
...
})(OverlayBondedExistingConcreteForm);

function mapStateToProps(state) {
    const subgradeSelector                = formValueSelector(OVERLAY_BONDED_CONCRETE_EXISTING_CONCRETE_FORM);
    ...
    const subgradeFormErrors              = getFormSyncErrors(OVERLAY_BONDED_CONCRETE_EXISTING_CONCRETE_FORM)(state);
    const structureFormErrors             = getFormSyncErrors(OVERLAY_BONDED_CONCRETE_SUBBASE_FORM)(state);
    ...
    const fatigueAdjustmentFactor         = subgradeSelector(state, FATIGUE_ADJUSTMENT_FACTOR);
    const durabilityAdjustmentFactor      = subgradeSelector(state, DURABILITY_ADJUSTMENT_FACTOR);
...
OverlayBondedExistingConcreteForm = connect(
    mapStateToProps, mapDispatchToProps
)(OverlayBondedExistingConcreteForm)

export default OverlayBondedExistingConcreteForm;
