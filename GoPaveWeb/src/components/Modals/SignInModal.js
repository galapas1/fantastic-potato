...
import { signInUser, setAuthErrorMessage, authSuccessMessage } from 'Actions';
...
class Signin extends Component {
  ...
  handleFormSubmit( {email, password}) {
    const {setAuthErrorMessage, signInUser, callback } = this.props;
    setAuthErrorMessage('');
    ...
    if (isFetching) {
       return (
         <div style={{display: 'flex', width: '85%'}}>
            ...
            <button disabled={true} className='btn btn-primary signup-button' action='submit'>Sign In</button>
         </div>
       )
     } else {
      return (
        <div style={{display: 'flex', width: '85%'}}>
            ...
            <button className='btn btn-primary signup-button' action='submit'>Sign In</button>
         </div>
      )
     }
  }
      ...
      <div>
        <ModalWrapper {...this.props} showClose={true} modalStyle={modalStyle}>
         ...
         <form className="signin-form" onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
            <label className="signup-label">LOG IN</label>
            ...
            <div onClick={() => showModal(FORGOT_PASSWORD_MODAL)} style={{color: '#3299CC', cursor: 'pointer', marginLeft: 3, fontStyle: 'italic'}} >Forgot Password</div>
